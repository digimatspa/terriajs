...
ArchesBIMCatalogGroup.prototype._createCatalogItem = function(result) {
  ...
  var catalogItem = new GltfCatalogItem(this.terria, url);

  //position
  var geojson = result._source.tiles[0].data[this.positionId];
  try {
    if (geojson) {
      ...
      catalogItem.name = result._source.tiles[0].data[this.itemNameId];
      return catalogItem;
    }
  } catch (e) {
    return null;
