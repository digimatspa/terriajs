...
var GeoJsonCatalogItem = function(terria, url) {
  ...
  /**
   * Gets or sets a value indicating whether the features in this GeoJSON should be clustered.
   * @type {Boolean}
   */
  ...
  /**
   * Gets or sets the cluster icon color
   * @type {String}
   * @default #778899
   */
  this.clusterColor = "#778899";
 ...
 */
GeoJsonCatalogItem.prototype.startPolling = function() {
...
GeoJsonCatalogItem.prototype._load = function() {
  ...
  require.ensure(
    ...
    function() {
      ...
      window.dataSource = that._dataSource;

      if (that.enableClustering) {
        var pinColor = Color.fromCssColorString(that.clusterColor);

        var pinBuilder = new PinBuilder();
        var pin50 = pinBuilder.fromText("50+", pinColor, 48).toDataURL();
        var pin40 = pinBuilder.fromText("40+", pinColor, 48).toDataURL();
        var pin30 = pinBuilder.fromText("30+", pinColor, 48).toDataURL();
        var pin20 = pinBuilder.fromText("20+", pinColor, 48).toDataURL();
        var pin10 = pinBuilder.fromText("10+", pinColor, 48).toDataURL();

        var singleDigitPins = new Array(8);
        for (var i = 0; i < singleDigitPins.length; ++i) {
          singleDigitPins[i] = pinBuilder
            .fromText("" + (i + 2), pinColor, 48)
            .toDataURL();
        }

        that._dataSource.clustering.clusterEvent.addEventListener(function(
          clusteredEntities,
          cluster
        ) {
        ...
        that._dataSource.clustering.pixelRange = that.pixelRange;
        that._dataSource.clustering.minimumClusterSize =
          that.minimumClusterSize;
      }
...
function loadGeoJson(geoJsonItem) {
  ...
  return dataSource.load(geoJsonItem._readyData, options).then(function() {
    ...
    for (var i = 0; i < entities.length; ++i) {
      ...
      if (
        ...
        entity.billboard = {
          ...
          color: Color.WHITE,
          eyeOffset: new Cartesian3(0.0, 0.0, -20.0),
          scaleByDistance: new NearFarScalar(50, 1.0, 1.5e5, 0.5)
        };
      ...
      if (geoJsonItem.showTitles && defined(properties["title"])) {
        entity.label = {
          text: properties["title"],
          font: "100 30px Helvetica",
          scale: 0.5,
    ...
    window.setTimeout(function() {
      var pr = dataSource.clustering.pixelRange;
      dataSource.clustering.pixelRange = 0;
      dataSource.clustering.pixelRange = pr;
    }, 3000);
    geoJsonItem.startPolling();
  });
}
...
function updateOpacity(item) {
  ...
  for (var i = 0; i < entities.length; i++) {
    ...
    if (defined(entity.billboard) && defined(entity.billboard.color)) {
      entity.billboard.color = entity.billboard.color
        .getValue()
        .withAlpha(item.opacity);
    }

    if (defined(entity.point) && defined(entity.point.color)) {
    ...
    if (
      defined(entity.polyline) &&
      defined(entity.polyline.material) &&
      defined(entity.polyline.material.color)
    ) {
    ...
    if (defined(entity.polygon) && defined(entity.polygon.material)) {
      entity.polygon.material.color = entity.polygon.material
        .getValue()
        .color.withAlpha(item.opacity);
    }
    if (defined(entity.polygon) && defined(entity.polygon.outlineColor)) {
