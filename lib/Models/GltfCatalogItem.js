...
/*global require*/
const CesiumColor = require("terriajs-cesium/Source/Core/Color").default;
const CesiumColorBlendMode = require("terriajs-cesium/Source/Scene/ColorBlendMode")
  .default;
...
function GltfCatalogItem(terria, url) {
  ...
  /**
   * Defines different modes for blending between a target color and a primitive's source color. HIGHLIGHT multiplies
   * the source color by the target color REPLACE replaces the source color with the target color MIX blends the source
   * color and target color together
   * @type {String}
   * @default null
   */
  this.colorBlendMode = undefined;
  ...
  /**
   * Only used when blend mode is MIX
   */
...
GltfCatalogItem.prototype._enableInCesium = function() {
  ...
  function getColor(colorCSS, alpha) {
    var color = CesiumColor.fromCssColorString(colorCSS);
    return CesiumColor.fromAlpha(color, parseFloat(alpha));
  }

  function getColorBlendMode(colorBlendMode) {
    return CesiumColorBlendMode[colorBlendMode.toUpperCase()];
  }
  ...
  if (this.colorBlendMode) {
    var blendMode = getColorBlendMode(this.colorBlendMode);
    options["colorBlendMode"] = blendMode;
    if (this.colorBlendMode.toUpperCase() === "MIX") {
      options["colorBlendAmount"] = parseFloat(this.colorBlendAmount);
    }
  }

  if (this.color) {
    options["color"] = getColor(this.color, this.colorAlpha);
  }

  if (this.scale) {
    options["scale"] = parseFloat(this.scale);
  }
