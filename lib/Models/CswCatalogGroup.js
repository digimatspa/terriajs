...
function createItemForUri(catalogGroup, record, uri, downloadUrls, legendUrl) {
  ...
  if (defined(catalogItem)) {
    if (catalogItem instanceof WebProcessingServiceCatalogGroup) {
      ...
      catalogItem.tokenUrl = catalogGroup.tokenUrl;
      catalogItem.tokenParameterName = catalogGroup.tokenParameterName;
      ...
      if (catalogItem.hasOwnProperty("layers")) {
        if (defined(layerName)) {
          catalogItem.layers = layerName;
        } else if (defined(record.alternative)) {
          catalogItem.layers = record.alternative;
        }
      }
