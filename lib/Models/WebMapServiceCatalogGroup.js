...
WebMapServiceCatalogGroup.prototype._load = function() {
  var terria_params = JSON.parse(decodeURIComponent(window.terria_params));
  var url =
    cleanAndProxyUrl(this, this.url) +
    `?service=WMS&request=GetCapabilities&version=1.3.0&tiled=true&access_token=${
      terria_params.TOKEN.token
    }`;

  // console.log(var allBaseMaps = australiaBaseMaps.concat(globalBaseMaps););
...
function createWmsDataSource(
  ...
  result.tokenUrl = terria_params.TOKEN_URL;
  result.tokenParameterName = terria_params.TOKEN_QUERY_PARAM_NAME;
