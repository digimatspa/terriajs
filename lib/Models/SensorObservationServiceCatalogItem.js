  ...
  knockout.getObservable(this, "activeConcepts").subscribe(function() {
    ...
    if (this.isLoading) {
      ...
      var error = new TerriaError({
        ...
        message:
          "Your data is still loading. You will be able to change the display once it has loaded."
      });
  ...
  function extractValues(response) {
    ...
    if (defined(observationData)) {
      ...
      observations.forEach(observation => {
        ...
        var measurements = [];
        //WaterML case
        if(defined(observation.result.MeasurementTimeseries)) {
        ...
        } else if (defined(observation.phenomenonTime) && defined(observation.phenomenonTime.TimeInstant) && defined(observation.phenomenonTime.TimeInstant.timePosition)) {
          //SensorML case
          measurements.push({
                time: observation.phenomenonTime.TimeInstant.timePosition,
...
SensorObservationServiceCatalogItem.prototype.loadIntoTableStructure = function(
  ...
  for (
    ...
    var additionalParamArray = convertObjectToNameValueArray({
    ...
    const templateContext = {
      ...
      additionalParameters: additionalParamArray,
...
function createMappingFromFeatureMembers(featureMembers) {
  ...
  featureMembers.forEach(member => {
    ...
    if (defined(member.MonitoringPoint)){
      monitoringPoint = member.MonitoringPoint;
    } else if  (defined(member.SF_SpatialSamplingFeature)){
      monitoringPoint = member.SF_SpatialSamplingFeature;
    } else {
        throw new DeveloperError(
    ...
    var shape = monitoringPoint.shape;
    ...
    if (defined(shape.Point)) {
      ...
      if (defined(posString.split)) {
        ...
        if (coords.length === 2) {
          var identifier = monitoringPoint.identifier.toString();
          mapping[identifier] = {
            lat: coords[0],
            lon: coords[1],
            name: monitoringPoint.name,
            id: monitoringPoint["gml:id"],
            identifier: identifier,
            type:
              monitoringPoint.type &&
              monitoringPoint.type["xlink:href"]
          };
