...
var CesiumColor = require("terriajs-cesium/Source/Core/Color").default;
var CesiumDirectionalLight = require("terriajs-cesium/Source/Scene/DirectionalLight")
  .default;
...
TerriaViewer.prototype.selectCesium = function() {
  ...
  require.ensure(
    ...
    function() {
      ...
      //debug
      window.viewer = that.terria.cesium;
      //window.viewer.scene.primitives.add(Cesium.prototype.createOsmBuildings());
      initializaCustomLight(that);
...
function initializaCustomLight(terriaViewer) {
  if (terriaViewer.terria.configParameters.customColorLight) {
    var scene = terriaViewer.terria.cesium.scene;
    scene.globe.enableLighting = true;

    scene.light = new CesiumDirectionalLight({
      direction: new Cartesian3(
        -0.2454278300540191,
        0.8842635425193919,
        0.39729481195458805
      ),
      color: CesiumColor.fromCssColorString(
        terriaViewer.terria.configParameters.customColorLight
      )
    });
  }
}
