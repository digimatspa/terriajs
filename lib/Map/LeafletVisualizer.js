...
LeafletGeomVisualizer.prototype._updatePoint = function(
  ...
  if (!defined(layer)) {
    var pointOptions = {
      ...
      fillColor: typeof color === "string" ? color : color.toCssColorString(),
...
LeafletGeomVisualizer.prototype._updateBillboard = function(
  ...
  var rotation = Property.getValueOrDefault(
  ...
  if (redrawIcon) {
    var applyRotation = function (icon, rotationAngle, rotationOrigin) {

      icon.style[L.DomUtil.TRANSFORM+'Origin'] = rotationOrigin;
      
      var oldIE = (L.DomUtil.TRANSFORM === 'msTransform');
      if(oldIE) {
          // for IE 9, use the 2D rotation
          icon.style[L.DomUtil.TRANSFORM] = 'rotate(' + rotationAngle + 'deg)';
      } else {
          // for modern browsers, prefer the 3D accelerated version
          icon.style[L.DomUtil.TRANSFORM] += ' rotateZ(' + rotationAngle + 'deg)';
      }
    ...
    var drawBillboard = function(image, dataurl) {
      ...
      if(rotation != null && rotation != 0){
        applyRotation(marker._icon, rotation, "50% 100%");
      }
    };
    var img = new Image();
    img.onload = function() {
